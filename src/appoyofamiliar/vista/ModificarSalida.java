 ...
 */
public class ModificarSalida extends javax.swing.JFrame {
    private LinkedList<Empleado> empList = new LinkedList<Empleado>();
    private LinkedList<Usuario> usuarios = new LinkedList<Usuario>();
    private GestionarSalidasEncargado gse;
    private Salida s;
    
    /**
     * Creates new form NuevoEncargado
     */
    public ModificarSalida() {
        initComponents();
    }
    
    public ModificarSalida(Salida s, GestionarSalidasEncargado gse) {
        initComponents();
        ...
        autoSeleccionarEmpleado();
        autoSleccionarPaciente();
        rellenarDatos();
    }
    
    private void rellenarDatos(){
        medicoBox.setText(s.getMedico());
        especialidadBox.setText(s.getEspecialidad());
        descripcionBox.setText(s.getDescripcion());
        centroBox.setText(s.getCentro());
        transporteBox.setText(s.getTransporte());
        areaBox.setText(s.getArea());
        fechaInicioBox.setText(s.getFechaInicio());
                
    }
    
    private void autoSeleccionarEmpleado(){
        int ind;
        boolean bandera = true;
        for (ind = 0; bandera && ind < empList.size(); ind++){
            if (empList.get(ind).getIdentificador().toLowerCase().equals(s.getEmpleado().getIdentificador().toLowerCase())){
                empleadoCombo.setSelectedIndex(ind);
                bandera = false;
            }
        }
    }
    
    private void autoSleccionarPaciente(){
        int ind;
        boolean bandera = true;
        for (ind = 0; bandera && ind < ConjuntoPaciente.instancia().getPacientes().size(); ind++){
            if (ConjuntoPaciente.instancia().getPacientes().get(ind).getDni().toLowerCase().equals(s.getPaciente().getDni().toLowerCase())){
                pacienteCombo.setSelectedIndex(ind);
                bandera = false;
            }
        }
    }
    ...
    private void botonAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonAceptarActionPerformed
        s.actualizarDatos(encontrarEmpleado(), encontrarPaciente(), medicoBox.getText(), especialidadBox.getText(), centroBox.getText(), areaBox.getText(), descripcionBox.getText(), transporteBox.getText(), fechaInicioBox.getText());
        gse.setVisible(true);
        this.setVisible(false);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ModificarSalida.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ModificarSalida().setVisible(true);
            }
        });
    }
